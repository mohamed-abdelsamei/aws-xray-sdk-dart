...
void main() {
  ...
  group('XRayTracer.runLambda — document shape', () {
    ...
    test('emitted doc has start_time and end_time', () async {
      ...
      expect((doc['end_time'] as double), greaterThanOrEqualTo(doc['start_time'] as double));
    });
  ...
  group('XRayTracer.runLambda — zone context', () {
    ...
    test('currentSegment carries the correct traceId inside runLambda', () async {
  ...
  group('XRayTracer.runLambda — subsegments', () {
    test('subsegments opened inside fn are embedded in the handler doc', () async {
      await tracer.runLambda(traceId, parentId, 'handler', () async {
    ...
    test('handler doc has no subsegments key when fn does no tracing', () async {
      await tracer.runLambda(traceId, parentId, 'handler', () async {});
